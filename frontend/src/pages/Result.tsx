...
const Result: React.FC = () => {
  ...
  console.log('Result page: Rendering with result from state:', result)
  ...
  if (!result) {
    console.log('Result: No result provided, skipping render')
    return (
      <div id="result-wrapper">
        <p style={{ textAlign: 'center', color: '#6c757d' }}>No result data available. Please try submitting again.</p>
        <button className="main-button" onClick={handleReset}>Try Again</button>
      </div>
    )
  }
  ...
  return (
    <div id="result-wrapper">
      ...
      <button id="reset-button" className="main-button" onClick={handleReset}>
        ประเมินอีกครั้ง
      </button>
    </div>
  )
