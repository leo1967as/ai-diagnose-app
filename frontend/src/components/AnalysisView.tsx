...
const AnalysisView: React.FC<AnalysisViewProps> = ({ result, onNavigateToResult, onReset }) => {
  console.log('AnalysisView: Rendering with props - result:', result)

  if (!result || !result.analysis) {
    return (
      <div id="analysis-wrapper">
        <p style={{ textAlign: 'center', color: '#6c757d' }}>ไม่มีข้อมูลการวิเคราะห์ กรุณาลองใหม่อีกครั้ง</p>
        <button className="main-button" onClick={onReset}>ลองใหม่</button>
      </div>
    )
  }
  ...
  return (
    <div id="analysis-wrapper" className="analysis-card">
      ...
      <div className="analysis-actions">
        {onNavigateToResult && (
          <button className="main-button secondary" onClick={onNavigateToResult}>
            ดูสรุปผลทั้งหมด
          </button>
        )}
        <button className="main-button" onClick={onReset}>
          ประเมินอีกครั้ง
        </button>
      </div>
    </div>
